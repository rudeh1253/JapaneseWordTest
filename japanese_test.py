...
class Note:  # 단어장 생성 및 불러오기
    ...
    def testReady(self):
        self.wordDict.clear()
        f = open(self.filet, "r", -1, "utf-8")
        index = 0 # random.shuffle(self.wordList)가 뒤 에 오므로 test 이후 정답과 비교를 위해 2차원 리스트에서 내부의 리스트 에 인덱스 넘버를 요소로 추가
        while True:
            line = f.readline()
            if not line :
                break
            line = line.rstrip()
            idx = 0
            line = removeRangeOf(string=line)
            line = line.replace(" ", "")
            line = line.replace("~", "")
            line = line.split(",")
            self.wordDict[index] = line
    ...
    def explain(self):
        ...
        print("wordNote.txt를 수정할 경우 다음과 같은  규칙을 따라야 합니다.\n"
              ...
              "2. 하나의 단어를 작 성하면 엔터를 눌러 줄바꿈을 해야 합니다. 줄바꿈이 각 단어를 구분짓는  지표입니다.\n"
...
def removeRangeOf(string, start='(', last=')'):
    ...
    while idx < len(string):
        if string[idx] == start:
            start_of = idx
        if string[idx] == last:
            string = string[:start_of] + string[idx + 1:]
            idx = 0
            continue
        idx += 1
...
def test(obj, k, l, m, txt):
    ...
    icount = 0  # icount : 틀린 개수
    ccount = 0  # ccount : 맞은 개수
    ...
    for i in length:  # test 시작
        ...
        if answer_hirakanji != obj.wordDict[i][l]:
            icount += 1
            if len(answer_hirakanji) == 0:
                obj.wordDict[i][l] = ""
            else:
                obj.wordDict[i][l] = answer_hirakanji
            if not removeRangeOf(tmp) in obj.wordDict[i]:
                obj.wordDict[i] = obj.wordDict[i][:2]
                if is_space:
                    obj.wordDict[i].append("")
                else:
                    for e in answer_sinifie.split(', '):
                        obj.wordDict[i].insert(len(obj.wordDict[i]) - 1, e)
            obj.wordDict[i].append("incorrect") # 틀린 부분의 리스트 요소에 추가
        elif not removeRangeOf(tmp) in obj.wordDict[i]:
            icount += 1
            obj.wordDict[i] = obj.wordDict[i][:2]
            if is_space:
                obj.wordDict[i].append("")
            ...
            ccount += 1
            obj.wordDict[i].append("correct")
    print("틀린 개수 : %d" % icount)
    print("맞은 개수 : %d\n" % ccount)
    wb = Workbook()
    ws_cor = wb.active
    ws_cor.title = "Correct"
    ws_incor = wb.create_sheet("Incorrect")
    for i in length:
        if "incorrect" in obj.wordDict[i]:
            obj.wordDict[i].remove("incorrect")
            print("입력 : ", end='')
            for p in obj.wordDict[i]:
                print(p, end=' ')
            print()
            print("정답 : ", end='')
            ...
            print("\n")
            temp_string = ""
            for idx in range(2, len(compare.wordDict[i]) - 1):
                temp_string += compare.wordDict[i][idx]
                temp_string += ", "
            temp_string += compare.wordDict[i][-1]
            compare.wordDict[i] = compare.wordDict[i][:2]
            compare.wordDict[i].append(temp_string)
            ws_incor.append(compare.wordDict[i])
        elif "correct" in obj.wordDict[i]:
            obj.wordDict[i].remove("correct")
            temp_string = ""
            for idx in range(2, len(compare.wordDict[i]) - 1):
                temp_string += compare.wordDict[i][idx]
                temp_string += ", "
            ...
            compare.wordDict[i] = compare.wordDict[i][:2]
            compare.wordDict[i].append(temp_string)
            ws_cor.append(compare.wordDict[i])
    obj.wordDict.clear()
    wb.save("result.xlsx")
    del compare
